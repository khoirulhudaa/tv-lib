...
export interface BaseDataTableProps {
  ...
  pageSize?: number;
  pageCount?: number; // Tambahkan prop untuk pageCount
  className?: string;
  rowClassName?: string;
  ...
  onPaginationChange?: (pagination: PaginationState) => void; // Tambahkan prop untuk onPaginationChange
}

export const BaseDataTableAttedance = ({
  ...
  actions,
  pageSize,
  pageCount, // Tambahkan pageCount
  onPaginationChange, // Tambahkan onPaginationChange
}: BaseDataTableProps) => {
  ...
  const table = useReactTable({
    ...
    pageCount: pageCount ?? -1, // Gunakan pageCount dari prop, fallback ke -1 jika undefined
    manualPagination: searchParamPagination, // Aktifkan server-side pagination
    state: {
      sorting,
      rowSelection,
      columnFilters,
  ...
  const handleNextPage = useCallback(() => {
    console.log('Klik Next, pageIndex saat ini:', table.getState().pagination.pageIndex);
    if (searchParamPagination && table.getCanNextPage()) {
      const newPageIndex = table.getState().pagination.pageIndex + 1;
      console.log('Mengubah ke pageIndex:', newPageIndex);
      setSearchParams({
        ...searchParamsToObject(searchParams.toString()),
        pageIndex: String(newPageIndex),
        pageSize: String(table.getState().pagination.pageSize),
      } as URLSearchParamsInit);
      return;
    }
    table.nextPage();
  ...
  const handlePrevPage = useCallback(() => {
    console.log('Klik Prev, pageIndex saat ini:', table.getState().pagination.pageIndex);
        ...
        <div className="flex items-center justify-end space-x-2 py-4">
          <div className="flex-1 text-sm text-muted-foreground">
            {`${table.getFilteredRowModel().rows.length} / ${
              pageCount ? pageCount * pagination.pageSize : data.length
            } ${lang.text("data")}`}
          </div>
          <div className="flex flex-row gap-2">
            <Button
